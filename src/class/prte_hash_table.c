...
static void prte_proc_table_construct(prte_proc_table_t* pt);
static void prte_proc_table_destruct(prte_proc_table_t* pt);

PRTE_CLASS_INSTANCE(
    prte_proc_table_t,
    prte_hash_table_t,
    prte_proc_table_construct,
    prte_proc_table_destruct
);

static void
prte_proc_table_construct(prte_proc_table_t* pt)
{
  pt->pt_size = 0;
}

static void
prte_proc_table_destruct(prte_proc_table_t* pt)
{
}
...
int prte_proc_table_init(prte_proc_table_t* pt, size_t jobids, size_t vpids) {
    int rc;
    if (PRTE_SUCCESS != (rc=prte_hash_table_init(&pt->super, jobids))) {
        return rc;
    }
    pt->vpids_size = vpids;
    return PRTE_SUCCESS;
...
int prte_proc_table_remove_all(prte_proc_table_t *pt) {
    int rc;
    prte_hash_table_t * vpids;
    uint32_t jobid;
    void * node;

    rc = prte_hash_table_get_first_key_uint32(&pt->super, &jobid, (void **)&vpids, &node);

    if (PRTE_SUCCESS == rc) {
        do {
            if (NULL != vpids) {
                prte_hash_table_remove_all(vpids);
                PRTE_RELEASE(vpids);
            }
            rc = prte_hash_table_get_next_key_uint32 (&pt->super, &jobid,
...
int prte_proc_table_get_value(prte_proc_table_t* pt, prte_process_name_t key,
                              void** ptr) {
    int rc;
    prte_hash_table_t * vpids;
    rc = prte_hash_table_get_value_uint32(&pt->super, key.jobid, (void **)&vpids);
    if (rc != PRTE_SUCCESS) {
        return rc;
    }
    rc = prte_hash_table_get_value_uint32(vpids, key.vpid, ptr);
    return rc;
}

int prte_proc_table_set_value(prte_proc_table_t* pt, prte_process_name_t key, void* value) {
    int rc;
    prte_hash_table_t * vpids;
    rc = prte_hash_table_get_value_uint32(&pt->super, key.jobid, (void **)&vpids);
    if (rc != PRTE_SUCCESS) {
        vpids = PRTE_NEW(prte_hash_table_t);
        if (NULL == vpids) {
            return PRTE_ERR_OUT_OF_RESOURCE;
        }
        if (PRTE_SUCCESS != (rc=prte_hash_table_init(vpids, pt->vpids_size))) {
            PRTE_RELEASE(vpids);
            return rc;
        }
        if (PRTE_SUCCESS != (rc=prte_hash_table_set_value_uint32(&pt->super, key.jobid, vpids))) {
            PRTE_RELEASE(vpids);
            return rc;
        }
    }
    rc = prte_hash_table_set_value_uint32(vpids, key.vpid, value);
    return rc;
}

int prte_proc_table_remove_value(prte_proc_table_t* pt, prte_process_name_t key) {
    int rc;
    prte_hash_table_t * vpids;
    if (PRTE_SUCCESS != (rc=prte_hash_table_get_value_uint32(&pt->super, key.jobid, (void **)&vpids))) {
        return rc;
    }
    if (PRTE_SUCCESS == (rc=prte_hash_table_remove_value_uint32(vpids, key.vpid))) {
        if (0 == vpids->ht_size) {
            prte_hash_table_remove_value_uint32(&pt->super, key.jobid);
            PRTE_RELEASE(vpids);
        }
    }
    return rc;
...
int prte_proc_table_get_first_key(prte_proc_table_t *pt, prte_process_name_t *key,
                                  void **value, void **node1, void **node2) {
    int rc;
    uint32_t jobid, vpid;
    prte_hash_table_t * vpids;
    if (PRTE_SUCCESS != (rc=prte_hash_table_get_first_key_uint32(&pt->super, &jobid, (void **)&vpids, node1))) {
        return rc;
    }
    rc = prte_hash_table_get_first_key_uint32(vpids, &vpid, value, node2);
    if (PRTE_SUCCESS == rc) {
        key->jobid = jobid;
        key->vpid = vpid;
    }
...
int prte_proc_table_get_next_key(prte_proc_table_t *pt, prte_process_name_t *key,
                                 void **value, void *in_node1, void **out_node1,
                                 void *in_node2, void **out_node2) {
    int rc;
    uint32_t jobid = ((prte_hash_element_t *)in_node1)->key.u32;
    uint32_t vpid;
    prte_hash_table_t * vpids = ((prte_hash_element_t *)in_node1)->value;

    rc = prte_hash_table_get_next_key_uint32(vpids, &vpid, value, in_node2, out_node2);
    if (PRTE_SUCCESS == rc) {
        key->jobid = jobid;
        key->vpid = vpid;
        *out_node1 = in_node1;
    ...
    if (PRTE_SUCCESS != (rc=prte_hash_table_get_next_key_uint32(&pt->super, &jobid, (void **)&vpids, in_node1, out_node1))) {
        return rc;
    }
    rc = prte_hash_table_get_first_key_uint32(vpids, &vpid, value, out_node2);
    if (PRTE_SUCCESS == rc) {
        key->jobid = jobid;
        key->vpid = vpid;
    }
